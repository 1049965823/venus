...
Guidelines for writing new hacking checks
 ...
 - Use only for Venus specific tests. OpenStack general tests
...
# NOTE(thangp): Ignore N323 pep8 error caused by importing venus objects
UNDERSCORE_IMPORT_FILES = ['./venus/objects/__init__.py']
...
class CheckLoggingFormatArgs(BaseASTChecker):
    ...
    LOG.debug("Venus %s caught fire and is at %d degrees C and climbing.",
              ('venus1', 500))
...
def no_test_log(logical_line, filename, noqa):
    if "venus/tests" not in filename or noqa:
        return
    # Skip the "integrated" tests for now
    if "venus/tests/unit/integrated" in filename:
        return
