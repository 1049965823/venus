...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./venus/tests"
    fi
  ...
  if [ setup.cfg -nt venus.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='venus/*' --omit='venus/openstack/common/*' -d covhtml -i
  fi
...
function run_pep8 {
  ...
  ${wrapper} tools/check_exec.py venus || exit 1
}
